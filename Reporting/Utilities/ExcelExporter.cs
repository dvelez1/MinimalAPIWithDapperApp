...
public static class DynamicExcelExporter
{
    ...
    public static void ExportMultipleListsToExcelAndSave(Dictionary<string, IEnumerable<object>> namedLists,string filePath)
    {
        using var workbook = new XLWorkbook();
    ...
    #endregion Export to Excel List 

    #region Export to Excel - DataTable 

    public static byte[] ExportDataTablesAndSaveOrReturn(Dictionary<string, DataTable> tables, string? filePath = null)
    {
        using var workbook = new XLWorkbook();
                    ...
                    else
                    {
                        switch (value)
                        {
                            ...
                            case float flt:
                                ...
                                break;
                            case bool b:
                                cell.SetValue(b ? "True" : "False");
                                break;
                            case Guid guid:
                                cell.SetValue(guid.ToString());
                                break;
                            case Enum e:
        ...
        {
            workbook.SaveAs(filePath);
            return Array.Empty<byte>(); // nothing to return
        }
        else
        {
            using var stream = new MemoryStream();
            workbook.SaveAs(stream);
            return stream.ToArray(); // return for download
        }
    }

    #endregion Export to Excel - DataTable 
